...
head(data)
hundata2019 <- subset(data, sex == "Hona" & year == 2019)  ##2019 hunbjørne
tail(hundata2019)

m <- n_distinct(hundata2019$id)   ##Antal obs individer
nid <- hundata2019 %>% count(id)
...
g <- nid %>% count(n) %>%
  ...
  labs(x = "Number of captures (poisson, 2019)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (poisgamma, 2019)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (poislognormal, 2019)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (2p-mixture, 2019)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (3p-mixture, 2019)", y = "Number of individuals") +
...
estimater[,2] <- c((238+369)/2-m, M_pois-m,M_gamma-m,M_logn-m,disc2pois_M-m,disc3pois_M-m)
estimater[,3] <- c(238-m,212-m,-154-m[], 304-m[], 206-m[], 226-m[])
estimater[,4] <- c(369-m, 240-m[],497-m[],352-m[],286-m[],309-m[])
estimater[,5] <- c(rep(2019,6))
est19 <- estimater ##Estimater af ikke-observerede bjørne
...
g <- ggplot(esttyve, aes(x = model, y = est, color = Models)) +        # ggplot2 plot with confidence intervals
  ...
  labs(x = "Models (2019)", y = "Estimates of non-observed bears (2019)") +
  scale_x_continuous(breaks = 1:6)
gplot2019 <- g + theme_bw(base_size = 18)
gplot2019
