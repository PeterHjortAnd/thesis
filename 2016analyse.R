...
head(data)
hundata2016 <- subset(data, sex == "Hona" & year == 2016)  ##hunbjørne
tail(hundata2016)

m <- n_distinct(hundata2016$id)   ##Antal obs individer
nid <- hundata2016 %>% count(id)
...
g <- nid %>% count(n) %>%
  ...
  labs(x = "Number of captures (poisson, 2016)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (poisgamma, 2016)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (poislognormal, 2016)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (2p-mixture, 2016)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (3p-mixture, 2016)", y = "Number of individuals") +
...
estimater[,2] <- c(506/2-m, M_pois-m,M_gamma-m,M_logn-m,disc2pois_M-m,disc3pois_M-m)
estimater[,3] <- c(463/2-m,217-m,207-m[], 231-m[], 200-m[], 220-m[])
estimater[,4] <- c(548/2-m, 231-m[],316-m[],282-m[],255-m[],278-m[])
estimater[,5] <- c(rep(2016,6))
est16 <- estimater ##Estimater af ikke-observerede bjørne
...
g <- ggplot(esttyve, aes(x = model, y = est, color = Models)) +        # ggplot2 plot with confidence intervals
  ...
  labs(x = "Models (2016)", y = "Estimates of non-observed bears (2016)") +
  scale_x_continuous(breaks = 1:6)
gplot2016 <- g + theme_bw(base_size = 18)
gplot2016
