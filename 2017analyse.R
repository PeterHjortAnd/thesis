...
head(data)
hundata2017 <- subset(data, sex == "Hona" & year == 2017)  ##2017 hunbjørne
tail(hundata2017)

m <- n_distinct(hundata2017$id)   ##Antal obs individer
nid <- hundata2017 %>% count(id)
...
g <- nid %>% count(n) %>%
  ...
  labs(x = "Number of captures (poisson, 2017)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (poisgamma, 2017)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (poislognormal, 2017)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (2p-mixture, 2017)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (3p-mixture, 2017)", y = "Number of individuals") +
...
estimater[,2] <- c(494-m, M_pois-m,M_gamma-m,M_logn-m,disc2pois_M-m,disc3pois_M-m)
estimater[,3] <- c(454-m,384-m,299-m[], 452-m[], 382-m[], 472-m[])
estimater[,4] <- c(553-m, 395-m[],845-m[],553-m[],458-m[],574-m[])
estimater[,5] <- c(rep(2017,6))
est17 <- estimater ##Estimater af ikke-observerede bjørne
...
g <- ggplot(esttyve, aes(x = model, y = est, color = Models)) +        # ggplot2 plot with confidence intervals
  ...
  labs(x = "Models (2017)", y = "Estimates of non-observed bears (2017)") +
  scale_x_continuous(breaks = 1:6)
gplot2017 <- g + theme_bw(base_size = 18)
gplot2017
