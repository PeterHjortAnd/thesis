...
summary(data)
head(data)
hundata2020 <- subset(data, sex == "Hona" & year == 2020)  ##2020 hunbjørne
tail(hundata2020)

m <- n_distinct(hundata2020$id)   ##Antal obs individer
nid <- hundata2020 %>% count(id)
...
g <- nid %>% count(n) %>%
  ...
  labs(x = "Number of captures (poisson, 2020)", y = "Number of individuals") +
...
(M_gamma <- m/(1-p0_gamma)) ##Estimat af antal hunbjørne i 2020

g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (poisgamma, 2020)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (poislognormal, 2020)", y = "Number of individuals") +
...
Emax<- function(dat,tau) {
  uniroot(f = function(lambda) mysum(tau*(dat/lambda -1/(1-exp(-lambda)))), interval = c(0.0001, 1000))[["root"]]
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (2p-mixture, 2020)", y = "Number of individuals") +
...
g <- nid %>% count(n) %>%
  ggplot(aes(x = n, y = nn)) + geom_col() + theme_bw() +
  labs(x = "Number of captures (3p-mixture, 2020)", y = "Number of individuals") +
...
estimater[,2] <- c((1000+886)/2-m, M_pois-m,M_gamma-m,M_logn-m,disc2pois_M-m,disc3pois_M-m)
estimater[,3] <- c(886-m,753-m[], 739-m[], 951-m[], 810-m[], 833-m[])
estimater[,4] <- c(1000-m, 773-m[],1877-m[],1109-m[],886-m[],982-m[])
estimater[,5] <- c(rep(2020,6))
est20 <- estimater ##Estimater af ikke-observerede bjørne
esttyve <- as.data.frame(estimater)
colnames(esttyve) <- c(c('model', 'est','lower','upper','year'))
Models <- c('off.','poisson','poisgamma','poislogn','2-point','3-point')

g <- ggplot(esttyve, aes(x = model, y = est, color = model)) +        # ggplot2 plot with confidence intervals
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = lower, ymax = upper)) +
  labs(x = "Models (2020)", y = "Estimates of non-observed bears (2020)") +
  scale_x_continuous(breaks = 1:6)
gplot2020 <- g + theme_bw(base_size = 18)
gplot2020
